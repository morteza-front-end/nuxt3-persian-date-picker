<template>
	...
	>
		...
		<div
			...
			@click="open"
		>
			<slot :value="props.modelValue" name="value">
				<div
					:class="{
				...
				>
					<label
						...
						:for="name"
						class="datepicker_label pointer-events-none absolute right-9 transition-all duration-200 text-gray-18"
					>
						{{ label }}
					</label>
...
watchEffect(() => {
	console.log("inputValue Watch:",inputValue)
	emit("update:modelValue", unref(inputValue))
})
watch(
	() => selectedDate.value,
	async (newValue, oldValue) => {
		console.log("new value model value=>",newValue)
		if (newValue !== oldValue) {
			try {
				setValue(newValue)
			}catch (e) {
				console.log('Error set Value',e)
			}
		}
	}
)
